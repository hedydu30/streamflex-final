...
switch (action) {
  case "dedup-videos": {
    ...
    // Récupérer toutes les vidéos par pages de 1000
    ...
    while (true) {
      ...
      for (const v of rows) {
        const urlKey = (v.original_url || "").trim();
        // Extraire le chemin du fichier depuis download_url
        let pathKey = "";
        try { pathKey = new URL(v.download_url || "").pathname; } catch {}

        const isDupeUrl = urlKey && seenUrls.has(urlKey);
        const isDupePath = pathKey && pathKey.length > 1 && seenPaths.has(pathKey);

        if (isDupeUrl || isDupePath) {
          toDelete.push(v.id);
        } else {
          if (urlKey) seenUrls.set(urlKey, v.id);
    ...
    if (toDelete.length === 0) {
      return new Response(
        JSON.stringify({ success: true, deleted: 0, total_checked: totalChecked }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    console.log(`[dedup] done: ${deleted} deleted / ${totalChecked} checked`);
    return new Response(
      JSON.stringify({ success: true, deleted, total_checked: totalChecked }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
